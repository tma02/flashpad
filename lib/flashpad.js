...
app.get('/', function (req, res) {
  res.render('index', { padId: (Math.random()+1).toString(36).substring(20) });
});
...
function setupSocketHandlers(nsp) {
  ...
  nsp.on('connection', function (socket) {
    ...
    socket.on('change', function (data) {
      ...
      nsp.file = results[0];
      nsp.emit('diff', { socketId: socket.id.split('#')[1], diff: data.diff });
    ...
    socket.on('changeCursor', function (data) {
      ...
      console.log(socket.id +":", socket.cursorDelta);
      ...
      if (updateCursorsFlag && updateCursorsOrigin === socket.id) {
        for (var socketIdx in nsp.sockets) {
          var socketObj = nsp.sockets[socketIdx];
          if (socketObj.id !== socket.id && socket.cursorPosition.row == socketObj.cursorPosition.row && socket.cursorPosition.column < socketObj.cursorPosition.column) {
            ...
            nsp.emit('changeRemoteCursor', { socketId: socketObj.id.split('#')[1], value: socketObj.cursorPosition });
          }
        }
