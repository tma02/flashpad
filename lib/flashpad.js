...
app.get('/pad/*', function (req, res) {
  ...
  initRoomNsp(roomId);
  res.render('pad', { title: 'Flashpad', path: req.url, url: production ? 'https://flashpad.herokuapp.com' : (staging ? 'https://flashpad-staging.herokuapp.com' : 'http://localhost') });
});

app.get('/slide/*', function (req, res) {
  var roomId = req.url.split('/')[(req.url.split('/').length - 1)];
  initRoomNsp(roomId);
  res.render('slide', { title: 'Flashslide', path: req.url, url: production ? 'https://flashpad.herokuapp.com' : (staging ? 'https://flashpad-staging.herokuapp.com' : 'http://localhost') });
});
