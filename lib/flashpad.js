...
window = {};
require('../static/diff_match_patch.js');
...
function setupSocketHandlers(nsp) {
  ...
  nsp.on('connection', function (socket) {
    ...
    socket.on('change', function (data) {
      var diff = dmp.diff_main(nsp.file, data.value, true);
      if (diff.length > 2) {
        dmp.diff_cleanupSemantic(diff);
      }
      var patch_list = dmp.patch_make(nsp.file, data.value, diff);
      patch_text = dmp.patch_toText(patch_list);
      nsp.file = data.value;
      nsp.emit('diff', { socketId: socket.id.split('#')[1], diff: patch_text });
      for (var socketIdx in nsp.sockets) {
        var socketObj = nsp.sockets[socketIdx];
