...
app.use('/xss', express.static(path.join(__dirname, '../node_modules/xss/dist/')));
...
app.get('/pad/*', function (req, res) {
  ...
  var newId = genId();
  initRoomNsp(roomId);
  res.render('pad', { title: 'Flashpad', newId: newId, path: req.url, roomId: roomId, url: production ? 'https://flashpad.herokuapp.com' : (staging ? 'https://flashpad-staging.herokuapp.com' : 'http://localhost') });
});
...
app.get('/slide/*', function (req, res) {
  ...
  var newId = genId();
  initRoomNsp(roomId);
  res.render('slide', { title: 'Flashslide', newId: newId, path: req.url, roomId: roomId, url: production ? 'https://flashpad.herokuapp.com' : (staging ? 'https://flashpad-staging.herokuapp.com' : 'http://localhost') });
});

function genId() {
  // TODO: Implement better ID gen
  var roomId = (Math.random()+1).toString(36).substring(2);
...
function setupSocketHandlers(nsp) {
  ...
  nsp.on('connection', function (socket) {
    ...
    socket.on('change', function (data) {
      ...
      nsp.file = results[0];
      nsp.emit('diff', { socketId: socket.id, diff: data.diff });
    ...
    socket.on('updateCursor', function (data) {
      ...
      nsp.emit('changeRemoteCursor', { socketId: socket.id, value: data.value });
      if (updateCursorsFlag && updateCursorsOrigin === socket.id) {
        for (var socketIdx in nsp.sockets) {
          var socketObj = nsp.sockets[socketIdx];
          if (socketObj.id !== socket.id) {
            if (socket.lastUpdatePosition.row <= socketObj.cursorPosition.row && socket.cursorDelta.row != 0) {
              if (socket.lastUpdatePosition.column < socketObj.cursorPosition.column) {
                socketObj.cursorPosition.row += socket.cursorDelta.row;
              }
              else if (socket.lastUpdatePosition.row < socketObj.cursorPosition.row) {
                socketObj.cursorPosition.row += socket.cursorDelta.row;
              }
            }
            if (socket.cursorPosition.row === socketObj.cursorPosition.row && socket.lastUpdatePosition.column < socketObj.cursorPosition.column) {
            ...
            nsp.emit('changeRemoteCursor', { socketId: socketObj.id, value: socketObj.cursorPosition });
          }
        }
    ...
    socket.on('selectionRange', function (data) {
      ...
      nsp.emit('changeRemoteSelection', { socketId: socket.id, value: socket.selectionRange });
    });
    socket.on('disconnect', function() {
      nsp.emitNames();
      nsp.emit('bye', socket.id);
